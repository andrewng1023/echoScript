...
# # Master Mix Recipe
#
# Perform GG reactions in a 2 µl total volume:
#
# 0.1 µl BsaI/BsmBI (Esp3I)
#
# 0.1 T4 DNA Ligase
#
# 0.2 10x Ligase Buffer + PEG
#
# 1.2 µl H20 (assume ~4 parts per rxn, can modify this if there is larger variance between parts)

# Do rxns in a 2 µl volume (Prepare a master mix based on the number of reactions,
            ...
            for plate in allsourcepl:
                for well in allsourcepl[plate]:
                    ...
                    if allsourcepl[plate][well].keys()[0] == part:
                        plasmidpicklist.loc[j] = [plate,well,'Destination[1]',coord[i],100] #hardcode in to do less than 96 GGs at once for now
                        allsourcepl[plate][well][part] -= .1
                        j+=1
        destinationloc[rawdf.loc[i].GoldenGate.rstrip()] = coord[i]
        #Assume a regular dead volume plate
        mmpicklist.loc[i] = ['Source[1]',coord[i/29],'Destination[1]',coord[i],1800]

    date = time.strftime("%Y%m%d")
    plasmidpicklist.to_csv(date+'plasmidpicklist.csv',index=False)
    mmpicklist.to_csv(date+'mmpicklist.csv',index=False)
    #Save the source plates to a file
    pkl.dump(allsourcepl, open(date + dname, "wb"),-1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', action='store', dest = 'platetype', type=str, default = None,
        help = 'Plate Type')
    parser.add_argument('-d', action='store', dest = 'dname', type=str, default = 'sourceplates.p',
